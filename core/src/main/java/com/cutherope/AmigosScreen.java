...
private void construirVerJugadores() {
    ...
    for (USER u : ranking) {
        ...
        if (Gdx.files.internal(rutaAvatar).exists()) {
            ...
            card.add(imgAvatar).width(50).height(50).padRight(10);
        }
        ...
        if (friendsManager.sonAmigos(usuario, u.getUsername())) {
            ...
            card.add(lblYa).width(120).center();
            ...
            card.add(lblPend).width(120).center();
        } else {
            ...
            card.add(btnAgregar).width(120).height(40);
        }
